...
class Game:
    def __init__(self, region):
        ...
        # image_data = pag.screenshot()
        # print(image_data)
        # exit()

        # self.screenShot_img= np.asarray(image_data)
        # print(self.screenShot_img)
        # cooped_img = self.[]
        # cv.imshow('full_screen', self.screenShot_img)
        # cv.waitKey(0)
        # cv.destroyAllWindows()
        self.top, self.bottom, self.left, self.right = region[0], region[1], region[2], region[3]
    ...
    def get_minimap_image(self, minimap):
        """
        Takes a picture of the application window.
        """
    ...
    def get_player_location(self):
        ...
        print('player location now', location, 'lenth: ', len(location))
        # exit()
        if len(location) > 0:
            return location[0] 
        else:
    ...
    def get_portal_location(self):
        """
        Returns the (x, y) position of the portal on the mini-map.
        """
        location = self.locate(RUNE_BGRA)
        return location[0] if len(location) > 0 else None
